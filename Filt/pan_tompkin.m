function [qrs_amp_raw,qrs_i_raw,delay,HR]=pan_tompkin(ecg,fs,gr)
real_time_draw = 0;
...
THR_SIG = max(ecg_m(1:2*fs))*1/4; % 0.25 of the max amplitude  JG
...
if real_time_draw == 1
    figure; plot(ecg_m); hold on; scatter(locs,pks,'k');
    r1 = scatter(0,0,'g','markerfacecolor','g'); r2 = plot(0,0,'r');
end
for i = 1 : length(pks)
    ...
    if pks(i) >= THR_SIG*4 % JG 1.5->2
        ...
        if skip == 0  % skip is 1 when a T wave is detected
            ...
            flag_i = [flag_i i];
            if real_time_draw == 1
                r1.XData = qrs_i; r1.YData = qrs_c;
                r2.XData = locs(2:i); r2.YData = THRS_buf;
                xlim([qrs_i(end)-3000 qrs_i(end)+3000]);
                pause(0.05);
                drawnow;
            end
    ...
    if length(flag_i) > 80 % th lv weighted update by JG
        THR_SIG = 0.2*THR_SIG + 0.8*median(THRS_buf(flag_i(end-80:end-1)),'omitnan');
    end
...
temp_HR = movmean(temp_HR, [5 5]);

if gr
plot(temp_HR,'r');
end

% temp_HR = temp_HR([1, 1:end, end]);
% temp(qrs_i_raw(1:end-1)) = temp2;
% [q1, q2] = envelope(temp,fs*100,'peak');
% plot(temp)
% plot(q1)
...
%     HR(i) = temp_HR(cnt);
% end
% temp_HR = 60./(diff(temp)/500);
for i = 1:length(temp_HR)-1
    HR(qrs_i_raw(i):qrs_i_raw(i+1)) = temp_HR(i);
end
HR(1:qrs_i_raw(1)) = temp_HR(1);
HR = HR';

%% overlay on the signals
if gr
    figure,az(1)=subplot(211);plot(ecg_h);title('QRS on Filtered Signal');axis tight;
    ...
    az(2)=subplot(212);plot(ecg_m);title('QRS on MVI signal and Noise level(black),Signal Level (red) and Adaptive Threshold(green)');axis tight;
    hold on,scatter(qrs_i,qrs_c,'m');
