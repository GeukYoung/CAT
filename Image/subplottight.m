function h = subplottight(n,m,i,op,left_margin)
if nargin < 5
    left_margin = 0.1;
end
...
elseif op == 2
    ...
    if length(i) > 1
        length_m = max(c)-min(c)+1;
        length_n = max(r)-min(r)+1;
        ax = subplot('Position', [(min(c)-1)/m+left_margin, (1-(max(r))/n), length_m/m*1-((min(c)-1)/m+left_margin), length_n/n]);
    else
        ax = subplot('Position', [(c-1)*(1-left_margin)/m+left_margin, 1-(r)/n, (1-left_margin)/m, 1/n]);
    end
