...
plot(tidal.time_insp(:), ...
            tidal.volume_insp(:),'-o'); hold on;
...
%         Tidal_normal = max([temp(temp<max(temp));Tidal_normal]); clear temp;
    ...
    if 1
        try
            delete(show_tidal);
            delete(ref_candidate);
            delete(ref_poinf);
        catch
        end
        subplot(1,5,1:4);
        show_tidal = plot(tidal.time_insp(cntTidal), ...
            tidal.volume_insp(cntTidal),'ro'); 
        xlim([tidal.time_insp(cntTidal)-180 tidal.time_insp(cntTidal)+180])
        ref_candidate = plot(tidal.time_insp(tidal_sel), ...
            tidal.volume_insp(tidal_sel),'go'); 
        ref_poinf = plot(tidal.time_insp(idx_ref), ...
            tidal.volume_insp(idx_ref),'rp'); 
        drawnow;
        pause(0.01);
        
        subplot(1,5,5);
        plot(temp(:,2),temp(:,1)); axis tight square;
        temp2 = temp(:,2)-idx_Tidal_normal;
        ans = temp2(temp2==0)
...
%                     tidal_sel(tidal_sel < tidal.time_insp(cntTidal) - rule.Epoch) = 0;
%                     if cnt_Section > 1
%                         tidal_sel(tidal_sel < time_section(cnt_Section)) = 0;
%                     end
%                     tidal_sel(tidal_sel > 0) = 1;
%                     
%                     if sum(tidal_sel) < 5
%                         tidal_sel = cntTidal:cntTidal+4;
%                     else
%                         tidal_sel = tidal_sel .* (1:length(tidal_sel))';
%                         tidal_sel(tidal_sel == 0) = [];
%                     end