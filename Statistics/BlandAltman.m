...
for groupi=1:params.numGroups
	if strcmpi(params.symbols,'Num')
		...
		ph=plot(cAH, data.set1((groupi-1)*params.numElementsPerGroup+(1:params.numElementsPerGroup)), data.set2((groupi-1)*params.numElementsPerGroup+(1:params.numElementsPerGroup)),...
			'o','MarkerFaceColor',[0 0.45 0.74],'MarkerSize',4);
...
[~, stats.swp] = swtest((data.maskedDifferences-stats.differenceMean)/stats.differenceSTD, 0.05);
stats.kurtosis = kurtosis(data.maskedDifferences);
stats.skewness = skewness(data.maskedDifferences,1);
stats.BA_CI.n = stats.N;
stats.BA_CI.dof = stats.BA_CI.n-1;
stats.BA_CI.mean = stats.differenceMean;
stats.BA_CI.sd = stats.differenceSTD;
stats.BA_CI.se_mean = ((stats.BA_CI.sd)^2/stats.BA_CI.n)^0.5;
stats.BA_CI.se_LOA = ((3*(stats.BA_CI.sd)^2)/stats.BA_CI.n)^0.5;
stats.BA_CI.t = tinv(0.975,stats.BA_CI.dof);
stats.BA_CI.CI_mean = stats.BA_CI.mean + stats.BA_CI.t*stats.BA_CI.se_mean*[-1 1];
stats.BA_CI.CI_LOA_u = stats.BA_CI.mean + 1.96*stats.BA_CI.sd + stats.BA_CI.t*stats.BA_CI.se_LOA*[-1 1];
stats.BA_CI.CI_LOA_l = stats.BA_CI.mean - 1.96*stats.BA_CI.sd + stats.BA_CI.t*stats.BA_CI.se_LOA*[-1 1];

% stats.BA_CI.CI_mean = -0.83 + stats.BA_CI.t*stats.BA_CI.se_mean*[-1 1];
% stats.BA_CI.CI_LOA_u = 15 + stats.BA_CI.t*stats.BA_CI.se_LOA*[-1 1];
% stats.BA_CI.CI_LOA_l = -16 + stats.BA_CI.t*stats.BA_CI.se_LOA*[-1 1];
...
for groupi=1:params.numGroups
	...
	if strcmpi(params.symbols,'Num')
		...
		ph = plot(baAH,ref,dif,'o','MarkerFaceColor',[0 0.45 0.74],'MarkerSize',4);
		set(ph,'markersize',params.markerSize);
	end
end
axis(baAH,'square')
...
switch upper(params.baStatsMode)
	case {'NORMAL','GAUSSIAN'}
		...
		plot(a(1:2),stats.differenceMean-stats.rpc*[1 1],':k')
		text(a(2),stats.differenceMean+stats.rpc, [mynum2str(stats.differenceMean+stats.rpc,2)],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize);
		text(a(2),stats.differenceMean,[mynum2str(stats.differenceMean,2)],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize);
		text(a(2),stats.differenceMean-stats.rpc, [mynum2str(stats.differenceMean-stats.rpc,2)],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize);
		if ~isGaussian(stats) 
...
for i=1:length(params.baInfo)
	switch upper(params.baInfo{i})
		case 'RPC', BAtext = [BAtext; ['{\bfLOA: ' mynum2str(stats.rpc,2) params.diffUnitsStr '}']];
		case 'RPC(%)', BAtext = [BAtext; ['{\bfLOA: ' mynum2str(stats.rpc,2) params.diffUnitsStr '} (' mynum2str(stats.rpcPercent,2) '%)']];
