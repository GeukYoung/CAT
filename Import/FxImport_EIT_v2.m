function [Data] = FxImport_EIT_v2(Path_main, fig)
...
%             Path(cnt.eit).eit = strcat(Path_main, '/', dirlist(cnt_file).name);
            Path(cnt.eit).eit = fullfile(Path_main, dirlist(cnt_file).name);
        end
    end
...
%             Path(cnt.voltage).voltage = strcat(Path_main, '/', dirlist(cnt_file).name);
            Path(cnt.voltage).voltage = fullfile(Path_main, dirlist(cnt_file).name);
        end
    end
...
%             Path(cnt.phase).phase = strcat(Path_main, '/', dirlist(cnt_file).name);
            Path(cnt.phase).phase = fullfile(Path_main,dirlist(cnt_file).name);
        end
    end
end
