function [EIT_data,t_hms,fs] = FxEIT_RawImport(EIT_data_path,~,num_data)
if nargin > 1
    op = 'raw';
else
    op = 'mag';
...
for cnt_file = 1:length(dirlist)
    if length(dirlist(cnt_file).name) > 4
        if strcmp(dirlist(cnt_file).name(end-2:end),'raw')
...
ss = floor(time_index);
ms = rem(time_index,1)*1000;
t_hms = datetime(str2double(data_name{1}(5:8)),str2double(data_name{1}(9:10)),str2double(data_name{1}(11:12)), ...
0,0,ss,ms);
...
cnt = 1;
for cnt_scan = 1:num_data
    file_path = strcat(EIT_data_path,'\',data_name{cnt_scan});
    fid = fopen(file_path);
    ...
    for cnt_scan2 = 1:scan_num
        ...
        temp(3,:) = temp(3,:).*256 + temp(4,:);      % 3. R data
        temp(4,:) = temp(5,:).*256 + temp(6,:);      % 4. Q data
