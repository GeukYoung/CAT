function [Data] = FxImport_EIT_v3(Path_main, fig)
...
if cnt.eit > 0
    if sum(idx.eit) ~= length(idx.eit)
        msgbox('EIT_bin data missing');
    end
end
if cnt.voltage > 0
    if sum(idx.voltage) ~= length(idx.voltage)
        msgbox('Voltage data missing');
    end
end
if cnt.phase > 0
    if sum(idx.phase) ~= length(idx.phase)  
        msgbox('Phase data missing');
    end
end
if cnt.ecg > 0
    if sum(idx.ecg) ~= length(idx.ecg)
        msgbox('ECG data missing');
    end
end
...
%               end
            ...
            if 1
                ...
                if cnt.voltage >= cnt_EIT
                    ...
                    fclose(fid);
                    scan_num = size(raw_data,1)/header.meas_num;
                    if mod(size(raw_data,1),header.meas_num) ~= 0
                        if cnt_EIT < total_EIT
                            disp('voltage data miss');
                        end
                        raw_data(floor(scan_num)*header.meas_num+1:end) = [];
                        scan_num = floor(scan_num);
                    end
                    if cnt_EIT < total_EIT && scan_num ~= header.scan_num
                            msgbox(['data missing: EIT_bin' num2str(cnt_EIT) ' (' num2str(header.scan_num - scan_num) ' scan)']);
            ...
            if 0
                ...
                if cnt.voltage >= cnt_EIT
                    ...
                    Data.EIT.phase(:,cnt_scan:cnt_scan+scan_num-1) = reshape(raw_data,header.meas_num,scan_num);
                    clear raw_data;
                end
            end
...
%         Data.EIT.t_hms = datetime(epochtime,'ConvertFrom','epochtime','TicksPerSecond',1e3);
        Data.EIT.t_hms = datetime(epochtime/1e3,'ConvertFrom','epochtime'); % old matlab version does not support 'TicksPerSecond'
