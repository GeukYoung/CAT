function [Step,Section] = FxParam_optPEEP(PEEP)
% DataSet_PEEP = [Tidal.PEEP];

%%
opt.th_section = 0.5; % threshold of PEEP section
opt.nTV_section = 3; % minimum TV number of PEEP section
opt.th_step = 0.8; % threshold of PEEP step
...
%     PEEP(cnt_TV) = DataSet_PEEP(cnt_TV); % data import
    ...
    if cnt_TV >= opt.min_nstep
        ...
        if diff_PEEP < opt.th_section % check PEEP level change over th lvl
            ...
            if flag.update_section == 1 % check section update
                if (Section.Pdiff(cnt.section) > opt.th_step) && (Section.Pdiff(cnt.section-1) > opt.th_step)
                    if Section.dir(cnt.section) == Section.dir(cnt.section-1) % check same direction (increase or decrease)
                        if flag.update_step
                            ...
                            Step(cnt.step).idx = [Step(cnt.step).idx Section.idx(cnt.section)];
                        end
                    else
                        if length(Step) == cnt.step
