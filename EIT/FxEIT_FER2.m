function [imdl,Proj_Mat] = FxEIT_FER2(elec_pos,op)
%% input check
if nargin < 1
    elec_pos(:,1) = cos(linspace(0,2*pi,17));
    elec_pos(:,2) = -sin(linspace(0,2*pi,17));
    elec_pos(end,:) = [];
    op = [];
end

if ~isfield(op, 'bnd_pos')
    bnd_pos=elec_pos;
else
    bnd_pos = op.bnd_pos;
end
...
if ~isfield(op, 'isKyunghee')
    op.isKyunghee=1;
end


% if ~isfield(op, 'mask_factor')
%     op.mask_factor=elec_pos;
% end

%% position normalization
m_E_posi = mean(bnd_pos(:,1));
bnd_pos(:,1) = bnd_pos(:,1) - m_E_posi;
elec_pos(:,1) = elec_pos(:,1) - m_E_posi;

m_E_posi = mean(bnd_pos(:,2));
bnd_pos(:,2) = bnd_pos(:,2) - m_E_posi;
elec_pos(:,2) = elec_pos(:,2) - m_E_posi;

max_E_psi = max(max(abs(bnd_pos)));
bnd_pos = bnd_pos./max_E_psi;
elec_pos = elec_pos./max_E_psi;

n_ch = length(elec_pos);

%% Mesh Gen
% option=[];
[Node,Element,erod,BndIndex,fnum,bnd]=func_MeshGeneration_Bd(bnd_pos,op.bd_n,op.thick,op.hmax,elec_pos);
%
...
if exist('mask_factor') ~= 0
    if any(mask_factor) ~= 0
        mask = FxEIT_mask(n_ch,mask_factor);
    else
        mask = FxEIT_mask(n_ch);
    end
else
    mask = FxEIT_mask(n_ch);
end
% Sensitivity=func_sensitivity_skip(Geom.Element,Geom.Node,Geom.erod,ones(mm,1),Nskip,isKyunghee);
[Sensitivity,Area]=func_sensitivity_skip_fast(Element,Node,erod,ones(mm,1),op.Nskip,op.isKyunghee);
Sensitivity(mask,:)=[];
imdl.Sensitivity = Sensitivity;
...
Proj_Mat=func_bd_artifact_elim_newnew(Sensitivity,indx_interior,op.bd_alpha);
% temp = [5e-10, 1e-10, 5e-11, 1e-11, 5e-12, 1e-12, 5e-13, 1e-13];
% for i = 1:10
%     Proj_Mat=func_bd_artifact_elim_newnew(Sensitivity,indx_interior,temp(i));
%     fileID = fopen(strcat('PM',num2str(i),'.bin'),'w');
%     fwrite(fileID,Proj_Mat,'double');
%     fclose(fileID);
% end
