function [Tidal,DataSet] = FxAT_Process_v1(DataSet)
% input
%   data : EIT data 
% output
...
%     Tidal(cnt).idx_insp_v = peaks.insp(cnt):(peaks.exp(cnt)-1);
    ...
    if isfield(DataSet,'Pressure')
        Tidal(cnt).PIP = max(DataSet.Pressure.Paw(peaks.exp(cnt):peaks.exp(cnt+1)))*.001;
        Tidal(cnt).PEEP = min(DataSet.Pressure.Paw(peaks.exp(cnt):peaks.exp(cnt+1)))*.001;
        Tidal(cnt).dP = Tidal(cnt).PIP - Tidal(cnt).PEEP;
        flow_temp = DataSet.Pressure.Paw2(peaks.exp(cnt):peaks.exp(cnt+1));
        Tidal(cnt).flow_offset = mean(flow_temp([1:5 end-4:end]));
%         flow_temp = flow_temp - Tidal(cnt).flow_offset;
        
        Tidal(cnt).TVi = sum(abs(flow_temp(flow_temp<0)))*.001;
        Tidal(cnt).TVe = sum(abs(flow_temp(flow_temp>0)))*.001;
...
%% Sub functions
    ...
    function [Im_RVD, sdRVD] = sFxEIT_RVD(DataSet,tp)
        ...
        t_global = find(wave_sigma_sum>(max(wave_sigma_sum)*opt.th_rvd),1,'first')/DataSet.EIT.fs;
        for cnt_RVD = 1:length(idx_lung)
            t_open(cnt_RVD) = find(wave_sigma(cnt_RVD,:)>(max(wave_sigma(cnt_RVD,:))*opt.th_rvd),1,'first')/DataSet.EIT.fs;
...
%         imagesc(reshape(Im_RVD,64,64)'); axis image off;
    ...
    function [Im_Popen, sdPopen] = sFxEIT_Popen(DataSet,tp)
        ...
        for cnt_RVD = 1:length(idx_lung)
            ...
            Im_Popen(idx_lung(cnt_RVD)) = DataSet.Pressure.Paw(tp(1) + idx_open(cnt_RVD)) * .001;
        end
