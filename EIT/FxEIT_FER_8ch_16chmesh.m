function [imdl,Proj_Mat] = FxEIT_FER_8ch_16chmesh(bd_n,thick,hmax,E_posi,E_posi2)
if nargin < 3
    bd_n=10;
    thick=0.05;
    hmax=0.05;
...
erod2 = erod;
% erod2([1 3 5 7 9 11 13 15]) = []
erod2([2 4 6 8 10 12 14 16]) = []
% %% Mesh Gen2
% option=[];
% [Node2,Element2,erod2,BndIndex2,fnum2,bnd2]=func_MeshGeneration_Bd(E_posi2,bd_n,thick,hmax,option);
% %
% indx_near_bd=find(fnum2==1);
% indx_interior=find(fnum2==2);
% % indx_near_bd=func_get_near_bd(Geom.Element,Geom.Node,Geom.BndIndex,8);
% 
% % Display Mesh result
% pix=zeros(size(Element2,1),1);
% pix(indx_near_bd)=1;
% pix(indx_interior)=2;
% figure(2000);clf;
% patch('Faces',Element2,'Vertices',Node2,'FaceVertexCData',pix,'FaceColor','flat','EdgeColor','None');
% axis normal image off
% 
...
% mask = [1 2 7 8 9 10 16 17 18 19 25 26 34 35 36 51 52 53 68 69 70 85 86 87 102 103 104 113 119 120 121 128 129 130 136 137 138 144 145 146 147 153 154 162 163 164 179 180 181 196 197 198 213 214 215 230 231 232 241 247 248 249 256];
mask = [1 2 8 9 10 11 18 19 20 27 28 29 36 37 38 45 46 47 54 55 56 63 64 57];
% Sensitivity=func_sensitivity_skip(Geom.Element,Geom.Node,Geom.erod,ones(mm,1),Nskip,isKyunghee);
[Sensitivity,Area]=func_sensitivity_skip_fast(Element,Node,erod2,ones(mm,1),Nskip,isKyunghee);
Sensitivity(mask,:)=[];
...
for i = 1:length(erod2);
    imdl.fwd_model.electrode(1,i).nodes = erod2(i);
