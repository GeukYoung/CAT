function [CoV, raw_CoV] = FxEIT_CoV2(Sigma,Node,Element,display_flag,maxscale)
%% prepare grid image
% display_flag = 1;
maxscale = 0.25;
% maxscale = 2550;
...
CoV = [CoVx CoVy];
raw_CoV = [raw_CoVx+FOV_h1 raw_CoVy+FOV_v1];
%% Display result
% try
%     if display_flag == 1
%         subplot(2,3,[1 4]);
...
try
    if display_flag == 1
        figure('pos',[200,200,390,200]);
        imagesc(Image, [0 maxscale]); axis image off; colormap hot
